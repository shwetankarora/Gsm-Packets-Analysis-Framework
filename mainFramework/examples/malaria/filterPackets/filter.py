...
class Person():
    ...
    def __init__(self, pid, i=[0, 0], cod=0, coi=0, rp=[0, 0]):
        self.pid = pid
        self.immunity = random.randint(i[0], i[1]) if i[0] and i[1] else random.randint(5, 16)
        self.cod = cod  # chance of death
        self.coi = coi if coi else random.random()  # chance of infecting
        # self.cor # chance of reinfection
        self.rp = random.randint(rp[0], rp[1]) if rp[0] and rp[1] else random.randint(20, 30)
...
def spreadingE(pop, epidemic):
    ...
    print(pa.coi)
    print(pa.rp)
    print(pa.infectionAfterExposure)
    print(pa.daysToSpread)
    ...
    for i in pbar(range(epidemic.incubation, epidemic.totalDays)):
        ...
        for piece in p[2]:
            ...
            try:
                ...
                temp[pieceId] = 0
            if not d_value and piece['tower']['zone'] != id_to_homezone[pieceId]:
                            ...
                            if decision(coi_working):
                                for pid, zone in id_to_homezone.items():
                                    if zone == current_zone:
                                        temp2 = persons[pid]
                                        if not temp2.getVariable('infected') and not temp2.getVariable('recovery'):
                                            temp2.set_infected()
                                            break
